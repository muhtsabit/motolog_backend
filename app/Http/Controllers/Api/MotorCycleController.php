...
class MotorCycleController extends Controller
{
    public function index($user_id)
    {
        ...
        foreach ($motorcycles as $motor) {
            ...
            $motor->component_last_services = $components->isEmpty()
                ? (object) []
                : $components;
        }
        ...
        if ($user && !empty($user->fcm_token)) {
            ...
            foreach ($masterKomponen as $namaKomponen => $batasKm) {
                ...
                if ($selisihKm >= $batasKm) {
                    ...
                    // 1. Kirim FCM push notification ke device
                    $this->sendFcmNotification($user->fcm_token, $title, $body);

                    // 2. Simpan jejak notifikasi ke tabel notifications MySQL
